...
def main():
    ...
    repo_url = "https://git.doit.wisc.edu/cdis/cs/courses/cs544/f24/main/-/raw/main/p6/"

    # Dockerfile  README.md  cassandra.sh  docker-compose.yml  src
    # inside src: ClientRecordTemps.py  ClientStationMax.py  ClientStationName.py  ClientStationSchema.py  ghcnd-stations.txt  station.proto  weather.parquet
    files = [
        "autograde.py",
        "cassandra.sh",
        "Dockerfile",
    ...
    # check for files
    required_files = ["src/server.py"]
    for file in required_files:
        if not Path(file).exists():
            print(f"Error: {file} not found in the repository.")
            sys.exit(1)

    # Run auto-grade
    subprocess.run(["python3", "-u", "autograde.py", "-v", "-k", "-c"])
